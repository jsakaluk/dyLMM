#' Extract Compound Symmetry Correlation (Rho) from a gls Model
#'
#' Extracts the compound symmetry correlation (rho) from a fitted \code{gls} model
#' with \code{corCompSymm}. For APIM fits (e.g., from [fitAPIMindist()] or
#' [fitAPIMdist()]), rho is the partial intraclass correlation--the residual
#' correlation between dyad members controlling for predictors. For [fitICC()]
#' (unconditional model), rho equals the ICC.
#'
...
#'
#' @return A numeric vector with `rho` and optionally `CI_low` and `CI_high`.
#'
#' @seealso [getICC()] for the ICC from the unconditional model; [fitAPIMindist()],
#'   [fitAPIMdist()] for APIM fits.
#'
#' @export
#'
#' @examples
...
getRho <- function(fit, ci = NULL) {
  ...
  if (is.null(int) || is.null(int$corStruct)) {
    warning("Could not compute confidence interval for rho. Returning point estimate only.")
    return(c(rho = rho))
  }

  cs <- int$corStruct
  if (length(cs) == 0) return(c(rho = rho))
  ...
  if (is.null(ci_lo) || is.null(ci_hi)) return(c(rho = rho))

  c(rho = rho, CI_low = as.numeric(ci_lo), CI_high = as.numeric(ci_hi))
}
